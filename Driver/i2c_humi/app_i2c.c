...
void IIC_PinRemap_Init(void) {
    ...
    // 获取当前 RB_PIN_I2C 位的值
    uint8_t current_mapping = (R16_PIN_ALTERNATE & (1 << 11)) >> 11;

    // 打印当前 IIC 映射情况
    if (current_mapping) {
        PRINT("old IIC map to PB21/PB20\n");
    } else {
        PRINT("old IIC map to PB12/PB13\n");
    }

    // 将 IIC 引脚映射到 PB21/PB20
    R16_PIN_ALTERNATE |= (1 << 11);

    // 获取当前 RB_PIN_I2C 位的值
    current_mapping = (R16_PIN_ALTERNATE & (1 << 11)) >> 11;

    // 打印当前 IIC 映射情况
    if (current_mapping) {
        PRINT("now IIC map to PB21/PB20\n");
    } else {
        PRINT("now IIC map to PB12/PB13\n");
    }
    ...
    // 初始化 IIC 控制器
    //I2C_Init(I2C_Mode_Master, I2C_ClockSpeed_100KHz);
}
...
void print_float_humidity(float value) {
     int integer_part = (int)value;
    int decimal_part = (int)((value - integer_part) * 100); // 保留两位小数

    if (decimal_part < 0) {
        decimal_part = -decimal_part; // 确保小数部分为正
    }

    PRINT("%d.%02d", integer_part, decimal_part);
}
...
{
    uint8_t cmd[2] = {SHT30_MEASURE_CMD >> 8, SHT30_MEASURE_CMD & 0xFF};
    uint8_t data[6];
    int ret;

    // Send measurement command to SHT30
    //PRINT("Send measurement command to SHT30\r\n");
    ret = i2c_write_to(SHT30_ADDR, cmd, sizeof(cmd), true, true);
    if (ret) {
        PRINT("Failed to send measure command to SHT30\n");
        return;
    }else {
        //PRINT("i2c_write_to Successes\n");
    }

    // Wait for measurement to complete
    //DelayMs(15);

    // Read 6 bytes of data from SHT30
    //PRINT("Read 6 bytes of data from SHT30\r\n");
    ret = i2c_read_from(SHT30_ADDR, data, sizeof(data), true, 1000);
    if (ret != 6) {
        PRINT("Failed to read data from SHT30\n");
        return;
    }
    else {
        //PRINT("i2c_read_from Successes\n");
    }

    //PRINT("Data: %#x %#x %#x %#x %#x %#x\n", data[0], data[1], data[2], data[3], data[4], data[5]);
    ...
    *humidity = 100 * (raw_hum / 65535.0);
    ...
    PRINT("Temperature: ");
    print_float_humidity(*temperature);
    PRINT(" C, Humidity: ");
    print_float_humidity(*humidity);
    PRINT(" %%\n");
...
__INTERRUPT
__HIGH_CODE
void I2C_IRQHandler(void)//I2C_IRQn
{
